...
exports.lezgiToNum = void 0;
const constants_1 = require("./constants");
function lezgiToNum(lezgiNum) {
    if (typeof lezgiNum !== 'string') {
        throw new TypeError('Provided value is not a string');
    }
    const isNegative = lezgiNum.startsWith(constants_1.MINUS);
    if (isNegative) {
        lezgiNum = lezgiNum.replace(constants_1.MINUS, '');
    }
    const lezgiNumeralArray = lezgiNum.trim().split(' ');
    if (lezgiNumeralArray.length === 1) {
        // Handle simple mapped numbers e.g. 'виш', 'кьвед' etc.
        if (constants_1.numerals[lezgiNum]) {
            if (constants_1.numerals[lezgiNum].requiresNext) {
                throw new Error(`Provided value '${lezgiNum}' requires a next value e.g. '${constants_1.numerals[lezgiNum].allowedNext ? constants_1.numerals[lezgiNum].allowedNext.minStr : 'сад'}'`);
            }
            return isNegative ? constants_1.numerals[lezgiNum].value * -1 : constants_1.numerals[lezgiNum].value;
        }
        else {
        ...
        const mappedNumeralArray = lezgiNumeralArray.map((lezgiNumeral) => {
            if (constants_1.numerals[lezgiNumeral]) {
                return constants_1.numerals[lezgiNumeral];
            }
            else {
                throw new Error(`Provided value is not a valid Lezgi numeral: '${lezgiNumeral}'`);
            }
        });
        const mappedNumeralsChunks = [mappedNumeralArray[0].value];
        if (mappedNumeralArray.length > 1) {
            for (let i = 1; i < mappedNumeralArray.length; i++) {
                ...
                if (previous.allowedNext &&
                    (curr.value < previous.allowedNext.min || curr.value > previous.allowedNext.max)) {
                    throw new Error(`In the provided value '${lezgiNum}' should be a number between '${previous.allowedNext.min}' and '${previous.allowedNext.max}' after '${lezgiNumeralArray[i - 1]}',` +
                        `but '${lezgiNumeralArray[i]}' was provided which equals to '${curr.value}'`);
                }
                if (curr.requiresNext && i === mappedNumeralArray.length - 1) {
                    throw new Error(`Provided value '${lezgiNum}' requires a next value, but none was provided`);
                }
